    ...
    def _load_master_key(self) -> bytes:
        ...
        if self.config.master_key_source == "env":
            ...
            try:
                ...
                raise ValueError(
                    f"Master key file not found: {self.config.master_key_file_path}"
                )
            ...
            raise ValueError(
                f"Unsupported master key source: {self.config.master_key_source}"
            )
    ...
    def _load_kms_key(self) -> bytes:
        ...
        try:
            ...
            response = kms.generate_data_key(
                KeyId=self.config.kms_key_id, KeySpec="AES_256"
            )
...
class StateEncryptor:
    ...
    def encrypt_state(self, state_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.config.algorithm == "fernet":
            ...
            raise ValueError(
                f"Unsupported encryption algorithm: {self.config.algorithm}"
            )
    ...
    def decrypt_sensitive_field(
        self, encrypted_value: str, context: str = "field"
    ) -> str:
