...
class CredentialSanitizer:
    ...
    CREDENTIAL_PATTERNS = [
        ...
        # Generic API keys
        re.compile(r'["\']?[a-zA-Z0-9_-]*(?:api[_-]?key|secret|token)["\']?\s*[:=]\s*["\']?([A-Za-z0-9/+=_-]+)["\']?', re.IGNORECASE),
    ]
    ...
    def sanitize_dict(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sensitive_keys = {
            ...
            'api_key', 'token', 'credential', 'auth', 'secret',
...
class CredentialManager:
    ...
    def get_credentials(self) -> CredentialInfo:
        ...
        if (self.current_credentials and 
            not self.current_credentials.is_expired() and
            not (self.config.auto_refresh_tokens and 
                 self.current_credentials.needs_refresh(self.config.token_refresh_threshold))):
    ...
    def _obtain_credentials(self) -> CredentialInfo:
        ...
        if self.config.use_environment_variables:
            try:
                ...
                logger.warning("Using environment variable credentials - consider IAM roles for better security")
                return credentials
            except Exception as e:
        ...
        if self.config.use_profile:
            try:
                ...
                logger.warning(f"Using profile credentials ({self.config.use_profile}) - consider IAM roles for better security")
                return credentials
            except Exception as e:
    ...
    def _assume_role(self) -> CredentialInfo:
        ...
        assume_role_kwargs = {
            ...
            'RoleSessionName': self.config.role_session_name or f'parsl-ephemeral-{int(time.time())}',
        ...
        if self.config.require_mfa:
            ...
            logger.info("MFA required for role assumption - implement interactive MFA flow")
    ...
    def _get_profile_credentials(self) -> CredentialInfo:
        ...
        try:
            ...
            if not credentials:
                raise NoCredentialsError(f"Profile {self.config.use_profile} not available")
    ...
    def get_credential_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "needs_refresh": self.current_credentials.needs_refresh(self.config.token_refresh_threshold)
        }
        ...
        if self.current_credentials.expiry_time:
            ...
            info["expires_in_seconds"] = int(
                (self.current_credentials.expiry_time - datetime.utcnow()).total_seconds()
            )
