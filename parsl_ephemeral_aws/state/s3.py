...
class S3State(StateStore):
    ...
    def __init__(self, 
                ...
                create_bucket_if_not_exists: bool = False) -> None:
        """Initialize S3 state.
            ...
            if error_code == '404':
                ...
                try:
                    ...
                    if self.provider.region == 'us-east-1':
                        ...
                        self.s3_client.create_bucket(
                            Bucket=self.bucket_name,
                            CreateBucketConfiguration={'LocationConstraint': self.provider.region},
                            ACL='private'
                        )
                    ...
                    self.s3_client.put_bucket_tagging(
                        ...
                        Tagging={
                            'TagSet': [
                                ...
                                {'Key': 'ParslWorkflowId', 'Value': self.provider.workflow_id}
                            ]
                        }
                    )
    ...
    def save_state(self, state_key: str, state_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.s3_client.put_object(
                ...
                Metadata={
                    'ParslWorkflowId': self.provider.workflow_id
                }
            )
        ...
        try:
            ...
            for page in page_iterator:
                for obj in page.get('Contents', []):
                    ...
                    object_key = obj['Key']
                    state_key = object_key[len(self.key_prefix) + 1:]  # +1 for the slash
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to load state data for {state_key}: {e}")
    ...
    def delete_bucket_if_empty(self) -> bool:
        ...
        try:
            ...
            if response.get('KeyCount', 0) > 0:
                logger.debug(f"S3 bucket {self.bucket_name} is not empty, skipping deletion")
                return False
