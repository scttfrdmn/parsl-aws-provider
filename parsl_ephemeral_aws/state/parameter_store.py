...
class ParameterStoreState(StateStore):
    ...
    def __init__(self, 
                ...
                use_secure_string: bool = False) -> None:
    ...
    def load_state(self, state_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if e.response['Error']['Code'] == 'ParameterNotFound':
                    logger.debug(f"State not found in Parameter Store: {parameter_name}")
                    return None
                raise
            ...
            for page in page_iterator:
                for parameter in page['Parameters']:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to parse state data for {state_key}: {e}")
        ...
        try:
            ...
            for i in range(0, len(parameters_to_delete), batch_size):
                ...
                if batch:
                    ...
                    logger.debug(f"Deleted {len(batch)} parameters from Parameter Store")
            
            logger.info(f"Cleaned up {len(parameters_to_delete)} workflow states from Parameter Store")
