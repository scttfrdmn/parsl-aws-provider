...
class VPCManager:
    ...
    def create_route_table(self, subnet_id: str, is_public: bool = True) -> str:
        ...
        try:
            ...
            logger.info(
                f"Created route table {route_table_id} and associated with subnet {subnet_id}"
            )
    ...
    def create_network_configuration(
        ...
        try:
            ...
            if not subnet_cidrs:
                ...
                subnet_cidrs = self._generate_subnet_cidrs(
                    DEFAULT_VPC_CIDR, num_subnets
                )
    ...
    def _generate_subnet_cidrs(self, vpc_cidr: str, num_subnets: int) -> List[str]:
        ...
        subnet_prefix_length = vpc_network.prefixlen + max(
            1, (num_subnets - 1).bit_length()
        )

        # Generate subnet CIDRs
        subnet_cidrs = [
            str(subnet)
            for subnet in vpc_network.subnets(new_prefix=subnet_prefix_length)
        ]
        ...
        try:
            ...
            try:
                ...
                if not response["RouteTables"]:
                    logger.warning(
                        f"Route table {route_table_id} not found, skipping cleanup"
                    )
                    return
            except ClientError as e:
            ...
            for route in route_table.get("Routes", []):
                ...
                try:
                    ...
                    logger.warning(
                        f"Error deleting route in route table {route_table_id}: {e}"
                    )
            ...
            raise ResourceCleanupError(
                f"Failed to delete route table {route_table_id}: {e}"
            )
        ...
        try:
            ...
            try:
                ...
                if e.response["Error"]["Code"] == "InvalidInternetGatewayID.NotFound":
                    logger.warning(
                        f"Internet gateway {self.igw_id} not found, skipping detach"
                    )
                    self.igw_id = None
                    return
                raise
            ...
            logger.info(
                f"Detached internet gateway {self.igw_id} from VPC {self.vpc_id}"
            )

        except ClientError as e:
            logger.error(f"Error detaching internet gateway {self.igw_id}: {e}")
            raise ResourceCleanupError(
                f"Failed to detach internet gateway {self.igw_id}: {e}"
            )
    ...
    def delete_internet_gateway(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if e.response["Error"]["Code"] == "InvalidInternetGatewayID.NotFound":
                    logger.warning(
                        f"Internet gateway {self.igw_id} not found, skipping delete"
                    )
                    self.igw_id = None
                    return
                raise
            ...
            raise ResourceCleanupError(
                f"Failed to delete internet gateway {self.igw_id}: {e}"
            )
