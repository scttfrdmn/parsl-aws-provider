...
class EC2Manager:
    ...
    def _setup_network_resources(self) -> Dict[str, str]:
        ...
        try:
            ...
            if self.provider.use_public_ips:
                ...
                logger.info(f"Enabled public IP assignment for subnet: {self.subnet_id}")
            ...
            for _ in range(count):
                ...
                for i in range(self.provider.nodes_per_block):
                    ...
                    if self.provider.use_spot_instances:
                        instance_id = self._create_spot_instance(node_id, network, block_id)
                    else:
                        instance_id = self._create_on_demand_instance(node_id, network, block_id)
    ...
    def _create_on_demand_instance(self, node_id: str, network: Dict[str, str], block_id: str) -> str:
        ...
        for key, value in self.provider.tags.items():
            instance_params['TagSpecifications'][0]['Tags'].append({'Key': key, 'Value': value})
    ...
    def _create_spot_instance(self, node_id: str, network: Dict[str, str], block_id: str) -> str:
        ...
        current_spot_price = float(spot_price_response['SpotPriceHistory'][0]['SpotPrice']) \
            if spot_price_response['SpotPriceHistory'] else None
        ...
        max_spot_price = str(current_spot_price * (self.provider.spot_max_price_percentage / 100.0))
        ...
        # Add provider tags
        for key, value in self.provider.tags.items():
            spot_params['TagSpecifications'][0]['Tags'].append({'Key': key, 'Value': value})
        ...
        logger.info(f"Spot request fulfilled with instance: {instance_id} for block {block_id}")
    ...
    def terminate_block(self, block_id: str) -> None:
        ...
        try:
            ...
            if instance_ids:
                ...
                logger.info(f"Terminated instances for block {block_id}: {instance_ids}")
            ...
            if response['Reservations'] and response['Reservations'][0]['Instances']:
                ...
                if instance_id in self.instances:
                    ...
                    if 'PublicIpAddress' in response['Reservations'][0]['Instances'][0]:
                        instance["public_ip"] = response['Reservations'][0]['Instances'][0]['PublicIpAddress']
                    
                    if 'PrivateIpAddress' in response['Reservations'][0]['Instances'][0]:
                        instance["private_ip"] = response['Reservations'][0]['Instances'][0]['PrivateIpAddress']
                ...
                'TagSpecifications': [
                    {
                        'ResourceType': 'instance',
                        'Tags': [
                            {'Key': 'Name', 'Value': f"{TAG_PREFIX}-bastion-{self.provider.workflow_id}"},
                            {'Key': TAG_NAME, 'Value': 'true'},
                            {'Key': TAG_WORKFLOW_ID, 'Value': self.provider.workflow_id},
    ...
    def cleanup_all_resources(self) -> None:
        ...
        try:
            ...
            if spot_request_ids:
                try:
                    self.ec2_client.cancel_spot_instance_requests(SpotInstanceRequestIds=spot_request_ids)
