...
class SpotInterruptionMonitor:
    ...
    def register_instance(self, instance_id: str, handler: Callable[[str, Dict[str, Any]], None]) -> None:
        ...
        logger.info(f"Registered spot instance {instance_id} for interruption monitoring")
    
    def register_fleet(self, fleet_request_id: str, handler: Callable[[str, List[str], Dict[str, Any]], None]) -> None:
        ...
        logger.info(f"Registered spot fleet {fleet_request_id} for interruption monitoring")
    ...
    def deregister_instance(self, instance_id: str) -> None:
        ...
        if instance_id in self.instance_handlers:
            ...
            logger.info(f"Deregistered spot instance {instance_id} from interruption monitoring")
    ...
    def deregister_fleet(self, fleet_request_id: str) -> None:
        ...
        if fleet_request_id in self.fleet_handlers:
            ...
            logger.info(f"Deregistered spot fleet {fleet_request_id} from interruption monitoring")
        ...
        try:
            ...
            for reservation in instances.get('Reservations', []):
                for instance in reservation.get('Instances', []):
                    ...
                    if instance.get('State', {}).get('Name') == 'marked-for-termination':
    ...
    def _check_fleet_interruptions(self, ec2_client) -> None:
        ...
        try:
            ...
            for fleet_id in fleet_request_ids:
                ...
                instance_ids = [instance['InstanceId'] for instance in fleet_instances.get('ActiveInstances', [])]
                ...
                if interrupted_instances:
                    ...
                    if handler:
                        ...
                        self.event_queue.put(('fleet', fleet_id, interrupted_instances, event_details))
                ...
                if event[0] == 'instance':
                    ...
                    if handler:
                        try:
                            ...
                            logger.error(f"Error in instance interruption handler for {instance_id}: {e}")
                    ...
                    if handler:
                        try:
                            ...
                            logger.error(f"Error in fleet interruption handler for {fleet_id}: {e}")
...
class SpotInterruptionHandler:
    ...
    def save_checkpoint(self, task_id: str, data: Dict[str, Any], priority: int = 1) -> str:
    ...
    def get_next_recovery_task(self) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            priority, timestamp, task_id, checkpoint_uri = self.recovery_queue.get_nowait()
            return {
                "task_id": task_id,
    ...
    def handle_instance_interruption(self, instance_id: str, event: Dict[str, Any]) -> None:
    ...
    def handle_fleet_interruption(
        ...
        """
        # This is a placeholder that should be overridden by subclasses
        logger.warning(f"Spot fleet {fleet_id} instances {instance_ids} are being interrupted: {event}")
...
class ParslSpotInterruptionHandler(SpotInterruptionHandler):
    ...
    def handle_instance_interruption(self, instance_id: str, event: Dict[str, Any]) -> None:
        ...
        logger.info(f"Found {len(tasks)} tasks to recover from interrupted instance {instance_id}")
        ...
        for task_id in tasks:
            try:
                ...
                if checkpoint_data:
                    self.queue_task_for_recovery(task_id, f"s3://{self.checkpoint_bucket}/{self.checkpoint_prefix}/{task_id}.json")
                else:
    ...
    def recover_tasks(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.info(f"Recovering task {task_id} using checkpoint {checkpoint_uri}")
