...
class SpotFleetManager:
            ...
            if self.provider.vpc_id:
                ...
                if self.provider.subnet_id:
                    ...
                    if response['Subnets']:
                        ...
                        logger.info(f"Found existing subnet {self.subnet_id} in VPC {self.vpc_id}")
                    else:
                        # Create a new subnet
                ...
                TagSpecifications=[
                    {
                        ...
                        'Tags': [
                            {'Key': 'Name', 'Value': f"{TAG_PREFIX}-sg-{self.provider.workflow_id}"},
                            {'Key': TAG_NAME, 'Value': 'true'},
                            {'Key': TAG_WORKFLOW_ID, 'Value': self.provider.workflow_id}
                        ]
                    }
        ...
        try:
            ...
            if self.security_group_id:
                try:
                    ...
                    logger.error(f"Error deleting security group {self.security_group_id}: {e}")
                self.security_group_id = None
            ...
            if self.vpc_id:
                try:
                    ...
                    logger.error(f"Error finding internet gateways for VPC {self.vpc_id}: {e}")
            ...
            if self.vpc_id:
                try:
                    ...
                    logger.error(f"Error finding route tables for VPC {self.vpc_id}: {e}")
    ...
    def _get_iam_fleet_role(self) -> str:
        ...
        try:
            ...
            logger.error(f"Error getting or creating IAM role for Spot Fleet: {error_code} - {error_msg}")
            ...
            if error_code == 'AccessDenied':
                ...
                logger.warning(f"IAM role {role_name} already exists but could not be retrieved")
                try:
                    # Try to get the role again
                    response = iam_client.get_role(RoleName=role_name)
                    self.iam_fleet_role_arn = response['Role']['Arn']
                    logger.info(f"Retrieved existing IAM role for Spot Fleet: {role_name}")
                    return self.iam_fleet_role_arn
                except Exception as retry_e:
                    logger.error(f"Failed to retrieve existing IAM role {role_name}: {retry_e}")
                    raise SpotFleetError(f"Failed to work with existing IAM role for Spot Fleet: {retry_e}")
            else:
                raise SpotFleetError(f"Failed to create IAM role for Spot Fleet: {error_code} - {error_msg}")
        except Exception as e:
            logger.error(f"Error getting or creating IAM role for Spot Fleet: {e}")
            raise SpotFleetError(f"Failed to get or create IAM role for Spot Fleet: {e}")
    ...
    def _generate_user_data(self) -> str:
        ...
        """
        user_data = "#!/bin/bash\n"
        user_data += f"echo 'Starting Parsl worker setup for {self.provider.workflow_id}'\n"
    ...
    def create_blocks(self, count: int) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            for _ in range(count):
                ...
                fleet_request_id = self._create_spot_fleet_request(block_id, network, self.provider.nodes_per_block)
    ...
    def _create_spot_fleet_request(self, block_id: str, network: Dict[str, str], target_capacity: int) -> str:
        ...
        instance_types = self.provider.instance_types if hasattr(self.provider, 'instance_types') else [
        ...
        for instance_type in instance_types:
            try:
                launch_spec = {
                    ...
                    'SecurityGroups': [
                        {'GroupId': network['security_group_id']}
                    ],
                    'UserData': base64.b64encode(self._generate_user_data().encode()).decode(),
                    'TagSpecifications': [
                        {
                ...
                for key, value in self.provider.tags.items():
                    launch_spec['TagSpecifications'][0]['Tags'].append({'Key': key, 'Value': value})
        ...
        for key, value in self.provider.tags.items():
            request_params['SpotFleetRequestConfig']['TagSpecifications'][0]['Tags'].append(
                {'Key': key, 'Value': value}
            )
        
        # Configure fleet to terminate instances when the request is cancelled
        request_params['SpotFleetRequestConfig']['TerminateInstancesWithExpiration'] = True
        ...
        if self.provider.spot_max_price_percentage:
            ...
            max_price = str(on_demand_price * (self.provider.spot_max_price_percentage / 100.0))
        ...
        try:
            ...
            logger.info(f"Created Spot Fleet request: {fleet_request_id} for block {block_id}")
            ...
            error_code = e.response['Error']['Code']
            error_msg = e.response['Error']['Message']
            logger.error(f"Error creating Spot Fleet request: {error_code} - {error_msg}")
            ...
            if error_code == 'RequestLimitExceeded':
                ...
                # IAM role issue
                raise SpotFleetRequestError(f"Invalid IAM role configuration: {error_msg}")
                ...
                raise SpotFleetRequestError(f"Insufficient capacity for Spot Fleet request: {error_msg}")
            else:
                # General Spot Fleet error
                raise SpotFleetError(f"Failed to create Spot Fleet request: {error_code} - {error_msg}")
    
    def _wait_for_fleet_instances(self, fleet_request_id: str, block_id: str, max_wait: int = 300) -> List[str]:
        ...
        while time.time() - start_time < max_wait:
            try:
                ...
                if status == 'active':
                    ...
                    raise ResourceCreationError(f"Spot Fleet request failed with status {status}")
                ...
                error_code = e.response['Error']['Code']
                error_msg = e.response['Error']['Message']
                logger.error(f"Error checking Spot Fleet status: {error_code} - {error_msg}")
                
                if error_code == 'RequestLimitExceeded':
                    # AWS is throttling our requests, back off and retry
                    retry_after = e.response.get('ResponseMetadata', {}).get('RetryAfter', 10)
                    logger.warning(f"AWS throttled request, backing off for {retry_after} seconds")
        ...
        if time.time() - start_time >= max_wait and not instance_ids:
            ...
            raise ResourceCreationError(f"Timeout waiting for Spot Fleet instances")
            ...
            if status == 'active':
                ...
                if not instance_ids:
                    ...
                    try:
                        ...
                        logger.error(f"Error getting instances for fleet {fleet_request_id}: {e}")
                ...
                if instance_ids:
                    ...
                    try:
                        ...
                        block_status = STATUS_RUNNING  # Assume running if we can't check
                else:
    ...
    def terminate_block(self, block_id: str) -> None:
        ...
        try:
            ...
            logger.info(f"Cancelled Spot Fleet request {fleet_request_id} for block {block_id}")
            ...
            error_code = e.response['Error']['Code']
            error_msg = e.response['Error']['Message']
            logger.error(f"Error terminating block {block_id}: {error_code} - {error_msg}")
            ...
            if error_code == 'RequestLimitExceeded':
                ...
                logger.warning(f"Spot Fleet {fleet_request_id} not found, may already be terminated")
                ...
                raise SpotFleetError(f"Failed to terminate Spot Fleet {fleet_request_id}: {error_code} - {error_msg}")
        except Exception as e:
        ...
        try:
            ...
            if fleet_request_ids:
                try:
                    ...
                    logger.info(f"Cancelled {len(fleet_request_ids)} Spot Fleet requests")
                except ClientError as e:
                    error_code = e.response['Error']['Code']
                    error_msg = e.response['Error']['Message']
                    logger.error(f"Error cancelling Spot Fleet requests: {error_code} - {error_msg}")
                    ...
                    if error_code == 'InvalidSpotFleetRequestId.NotFound':
                        logger.warning("Some Spot Fleet requests not found, trying to cancel individually")
                        for fleet_id in fleet_request_ids:
                            try:
                                ...
                                logger.warning(f"Could not cancel Spot Fleet {fleet_id}: {ind_error_code}")
                except Exception as e:
            ...
            if self.iam_fleet_role_arn:
                ...
                try:
                    ...
                    try:
                        ...
                        if e.response['Error']['Code'] == 'NoSuchEntity':
                            role_exists = False
                            logger.warning(f"IAM role {role_name} not found, skipping cleanup")
                        else:
                    ...
                    if role_exists:
                        ...
                        try:
                            ...
                            error_code = e.response['Error']['Code']
                            error_msg = e.response['Error']['Message']
                            logger.error(f"Error detaching policy from role {role_name}: {error_code} - {error_msg}")
                        ...
                        try:
                            ...
                            if error_code == 'DeleteConflict':
                                logger.warning(f"Cannot delete role {role_name} as it still has attached entities")
                                ...
                                try:
                                    ...
                                    for policy in attached_policies.get('AttachedPolicies', []):
                                        ...
                                        logger.info(f"Detached policy {policy['PolicyName']} from role {role_name}")
                                    
                                    # Try deletion again after detaching policies
                                    time.sleep(2)
                                    iam_client.delete_role(RoleName=role_name)
                                    logger.info(f"Deleted IAM role: {role_name} after detaching all policies")
                                except Exception as policy_e:
                                    logger.error(f"Error detaching policies from role {role_name}: {policy_e}")
                            elif error_code == 'NoSuchEntity':
                                logger.info(f"IAM role {role_name} already deleted")
                            else:
                                logger.error(f"Error deleting IAM role {role_name}: {error_code} - {error_msg}")
                except Exception as e:
        ...
        try:
            ...
            if response['Reservations'] and response['Reservations'][0]['Instances']:
                ...
                if instance_id in self.instances:
                    ...
                    self.instances[instance_id]['private_ip'] = instance.get('PrivateIpAddress')
        ...
        try:
            ...
            if response['Reservations'] and response['Reservations'][0]['Instances']:
                ...
                if instance_id in self.instances:
                    ...
                    self.instances[instance_id]['public_ip'] = instance.get('PublicIpAddress')
                    self.instances[instance_id]['status'] = instance['State']['Name']
