...
class ServerlessMode(OperatingMode):
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            if not self.vpc_id and self.create_vpc and (
                self.worker_type in [WORKER_TYPE_ECS, WORKER_TYPE_AUTO]
            ):
                self.vpc_id = self._create_vpc()
                
            # Create subnet if needed (for ECS tasks)
            if not self.subnet_id and self.vpc_id and (
                self.worker_type in [WORKER_TYPE_ECS, WORKER_TYPE_AUTO]
            ):
                self.subnet_id = self._create_subnet()
                
            # Create security group if needed (for ECS tasks)
            if not self.security_group_id and self.vpc_id and (
                self.worker_type in [WORKER_TYPE_ECS, WORKER_TYPE_AUTO]
            ):
    ...
    def _create_subnet(self) -> str:
        ...
        logger.info(f"Getting subnet from VPC stack")
    ...
    def submit_job(
        ...
        logger.info(f"Submitting job {job_id} ({job_name if job_name else 'unnamed'}) in serverless mode")
        ...
        try:
            ...
            if worker_type == WORKER_TYPE_LAMBDA:
                ...
                self._submit_ecs_job(job_id, command, tasks_per_node, job_name, resource_id)
    ...
    def _submit_lambda_job(
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Created CloudFormation stack {stack_name} for Lambda job {job_id}")
        ...
        try:
            ...
            self.cf_client.create_stack(
                ...
                Parameters=[
                    ...
                    {
                        'ParameterKey': 'VpcId',
                        'ParameterValue': self.vpc_id
                    },
                    {
                        'ParameterKey': 'SubnetIds',
                        'ParameterValue': self.subnet_id
                    },
                    {
                    ...
                    },
                    {
                        ...
                        'ParameterValue': 'ENABLED' if self.use_public_ips else 'DISABLED'
                    },
                    {
                        'ParameterKey': 'WorkflowId',
                        'ParameterValue': self.provider_id
                    ...
                    if worker_type == WORKER_TYPE_LAMBDA:
                        if self.lambda_manager:
                            ...
                            if function_name:
                                ...
                                status = self._get_lambda_status(function_name, resource_id)
                            else:
                        ...
                        if self.ecs_manager:
                            ...
                            if cluster_name and service_name:
                                ...
                                status = self._get_ecs_status(cluster_name, service_name)
                            else:
                                status = STATUS_RUNNING
                    else:
                        status = STATUS_RUNNING
            ...
            if task_arns:
                ...
                if task_details['tasks']:
                    ...
                    if last_status == 'PROVISIONING' or last_status == 'PENDING':
                        ...
                        if task.get('stoppedReason') and 'Essential container' in task.get('stoppedReason'):
    ...
    def cancel_jobs(self, resource_ids: List[str]) -> Dict[str, str]:
        ...
        for resource_id in resource_ids:
            ...
            try:
                ...
                logger.info(f"Cancelled job {resource.get('job_id')} (stack: {stack_name})")
                ...
                logger.error(f"Unexpected error cancelling job (stack: {stack_name}): {e}")
                cancel_map[resource_id] = STATUS_FAILED
    ...
    def cleanup_resources(self, resource_ids: List[str]) -> None:
        ...
        for resource_id in resource_ids:
            ...
            try:
                ...
                logger.info(f"Deleted stack {stack_name} for job {resource.get('job_id')}")
