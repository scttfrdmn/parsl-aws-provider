    ...
    try:
        ...
        for i, task in enumerate(tasks):
            try:
                ...
                logger.info(f"  CPU: {result['cpu_count']} cores, {result['cpu_percent']}% utilized")
                logger.info(f"  Memory: {result['memory_available_gb']:.2f}GB available of {result['memory_total_gb']:.2f}GB total")
                logger.info(f"  Execution time: {result['execution_time']:.2f} seconds")
                
                if result['compute_intensive']:
                    logger.info(f"  Computed {result['num_primes_calculated']} prime numbers")
