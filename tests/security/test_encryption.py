...
from parsl_ephemeral_provider.security.encryption import (
...
class TestEncryptionConfiguration:
    ...
    def test_file_key_source_validation(self):
        ...
        with pytest.raises(ValueError, match="master_key_file_path required"):
            EncryptionConfiguration(master_key_source="file", master_key_file_path=None)

        # Should not raise with file path
        config = EncryptionConfiguration(
            master_key_source="file", master_key_file_path="/path/to/key"
        )
...
class TestEncryptionKeyManager:
    ...
    @patch.dict(
        "os.environ", {"TEST_MASTER_KEY": "dGVzdC1rZXktMTIzNDU2Nzg="}
    )  # base64 encoded
    ...
    def test_load_master_key_missing_env(self):
        ...
        with pytest.raises(
            ValueError, match="Master key not found in environment variable"
        ):
    ...
    def test_load_master_key_from_file(self):
        ...
        with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
            ...
            config = EncryptionConfiguration(
                master_key_source="file", master_key_file_path=tmp_file.name
            )
            manager = EncryptionKeyManager(config)
    ...
    def test_load_master_key_file_not_found(self):
        ...
        config = EncryptionConfiguration(
            master_key_source="file", master_key_file_path="/nonexistent/key/file"
        )
        manager = EncryptionKeyManager(config)
    ...
    def test_load_kms_key(self, mock_boto3_client):
        ...
        mock_kms.generate_data_key.assert_called_once_with(
            KeyId=config.kms_key_id, KeySpec="AES_256"
        )
...
class TestStateEncryptor:
    ...
    def test_encrypt_decrypt_state_fernet(self):
        ...
        config = EncryptionConfiguration(
            algorithm="fernet", master_key_env_var="TEST_MASTER_KEY"
        )
    ...
    def test_encrypt_decrypt_state_aes_gcm(self):
        ...
        config = EncryptionConfiguration(
            algorithm="aes-gcm", master_key_env_var="TEST_MASTER_KEY"
        )
        encryptor = StateEncryptor(config)

        test_state = {"tasks": ["task-1", "task-2"], "cluster_name": "test-cluster"}
...
class TestSecureStateManager:
    ...
    def test_save_load_secure_state(self):
        ...
        test_state = {
            ...
            "resources": {"instances": ["i-123", "i-456"], "vpc_id": "vpc-123"},
        }
