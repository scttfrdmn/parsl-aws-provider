...
class TestWorkflowStatePersistence:
    ...
    def test_standard_mode_state_persistence(self, localstack_session, file_state_store, mock_ec2_client):
        ...
        with patch('boto3.Session', return_value=localstack_session):
            ...
            assert not os.path.exists(file_state_store.file_path) or os.path.getsize(file_state_store.file_path) == 0
    
    @pytest.mark.localstack
    def test_detached_mode_state_persistence(self, localstack_session, file_state_store):
            ...
            with patch.object(mode2, '_delete_ec2_instance'):
                with patch.object(mode2, '_delete_ssm_parameter'):
                    with patch.object(mode2, '_delete_security_group'):
                        with patch.object(mode2, '_delete_subnet'):
                            with patch.object(mode2, '_delete_vpc'):
                                mode2.preserve_bastion = False  # Ensure bastion is cleaned up
                                mode2.cleanup_infrastructure()
    ...
    @pytest.mark.localstack
    def test_serverless_mode_state_persistence(self, localstack_session, file_state_store):
        ...
        with patch('boto3.Session', return_value=localstack_session):
            ...
            with patch.object(mode.lambda_manager, '_create_lambda_function') as mock_create_lambda:
