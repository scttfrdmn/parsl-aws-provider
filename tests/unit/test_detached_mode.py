...
class TestDetachedMode:
    ...
    @pytest.fixture
    def detached_mode(self, mock_session, mock_state_store, mock_ec2_client, mock_cf_client, mock_ssm_client):
    ...
    def test_initialize_cloudformation(self, mock_get_cf_template, mock_get_default_ami, 
                                       detached_mode, mock_ec2_client, mock_cf_client):
    ...
    @patch('parsl_ephemeral_aws.modes.detached.get_default_ami')
    def test_initialize_direct(self, mock_get_default_ami, detached_mode, mock_ec2_client):
    ...
    def test_initialize_failure_cleanup(self, mock_delete_resource, detached_mode, mock_ec2_client):
    ...
    def test_cleanup_resources(self, detached_mode, mock_ssm_client):
        ...
        mock_ssm_client.delete_parameter.assert_any_call(
            Name=f"/parsl/workflows/test-workflow/jobs/job-1"
        )
        
        # Verify SSM delete_parameter was called for job status
        mock_ssm_client.delete_parameter.assert_any_call(
            Name=f"/parsl/workflows/test-workflow/status/job-1"
        )
    ...
    @patch('parsl_ephemeral_aws.modes.detached.delete_resource')
    def test_cleanup_infrastructure(self, mock_delete_resource, detached_mode, mock_ec2_client, mock_cf_client):
    ...
    def test_cleanup_infrastructure_preserve_bastion(self, detached_mode, mock_ec2_client):
