...
class TestFileStateStoreErrorHandling:
    ...
    def test_save_state_directory_not_exists(self, temp_dir):
        ...
        nested_path = os.path.join(
            temp_dir, "non_existent", "deeply", "nested", "state.json"
        )
...
class TestParameterStoreStateErrorHandling:
    ...
    def test_save_state_client_error(self, parameter_store_state, ssm_client_mock):
        ...
        ssm_client_mock.get_parameter.side_effect = ClientError(
            error_response, "GetParameter"
        )
        ssm_client_mock.put_parameter.side_effect = ClientError(
    ...
    def test_load_state_parameter_not_found(
        self, parameter_store_state, ssm_client_mock
    ):
    ...
    def test_delete_state_parameter_not_found(
        self, parameter_store_state, ssm_client_mock
    ):
        ...
        ssm_client_mock.delete_parameter.side_effect = ClientError(
            error_response, "DeleteParameter"
        )
...
class TestS3StateErrorHandling:
    ...
    def test_bucket_creation_error(
        ...
        s3_client_mock.create_bucket.side_effect = ClientError(
            error_response_create, "CreateBucket"
        )
    ...
    def test_cleanup_workflow_states_error(self, s3_state, s3_client_mock):
        ...
        error_response = {"Error": {"Code": "AccessDenied", "Message": "Access Denied"}}
        s3_client_mock.delete_objects.side_effect = ClientError(
