...
from parsl_ephemeral_aws.modes.base import ModeBase
...
class TestVPCWithMoto:
    ...
    def test_vpc_creation_and_deletion(self, moto_session):
        ...
        vpc_id = vpc_manager.create_vpc(
            ...
            tags={"Name": "test-vpc", "TestKey": "TestValue"}
        )
...
class TestEC2WithMoto:
    ...
    def test_ec2_instance_lifecycle(self, moto_session):
        ...
        assert status in ["pending", "running", "shutting-down", "terminated", "stopping", "stopped"]
...
class TestParameterStoreWithMoto:
    ...
    def test_parameter_store_lifecycle(self, moto_session):
        ...
        param_store = ParameterStoreState(
        ...
        assert loaded_state["resources"]["resource-1"]["id"] == test_state["resources"]["resource-1"]["id"]
...
"""
        ...
        with patch.object(lambda_manager, '_generate_lambda_code', return_value=b'dummy_code'):
...
class TestECSWithMoto:
    ...
    def test_ecs_cluster_lifecycle(self, moto_session):
        ...
        response = ecs_client.describe_clusters(clusters=[cluster_name])
        assert len(response["clusters"]) == 0 or response["clusters"][0]["status"] == "INACTIVE"
    ...
    def test_ecs_task_definition(self, moto_session):
        ...
        assert response["taskDefinition"]["containerDefinitions"][0]["name"] == "test-container"
...
class TestSpotFleetWithMoto:
    ...
    def test_spot_fleet_request_lifecycle(self, moto_session):
            ...
            "TagSpecifications": [
                {
                    ...
                    "Tags": [
                        {"Key": "Name", "Value": "test-spot-instance"}
                    ]
                }
            ]
        }
        ...
        with patch.object(spot_fleet_manager.ec2_client, 'request_spot_fleet') as mock_request:
        ...
        with patch.object(spot_fleet_manager.ec2_client, 'describe_spot_fleet_requests') as mock_describe:
            ...
            # Check status
            status = spot_fleet_manager.get_spot_fleet_request_status(spot_fleet_request_id)
            assert status == "active"
        
        # Mock cancel spot fleet request
        with patch.object(spot_fleet_manager.ec2_client, 'cancel_spot_fleet_requests') as mock_cancel:
...
class TestServerlessModeWithMoto:
    ...
    def test_serverless_mode_initialization(self, moto_session):
        ...
        with patch('parsl_ephemeral_aws.compute.lambda_func.LambdaManager') as mock_lambda_manager, \
             patch('parsl_ephemeral_aws.compute.ecs.ECSManager') as mock_ecs_manager, \
             patch('parsl_ephemeral_aws.network.vpc.VPCManager') as mock_vpc_manager, \
             patch('parsl_ephemeral_aws.network.security.SecurityGroupManager') as mock_sg_manager:
            
            # Create mock manager instances
            mock_lambda_manager.return_value.create_lambda_function.return_value = {"FunctionName": "test-function"}
            mock_ecs_manager.return_value.create_cluster.return_value = "test-cluster-arn"
