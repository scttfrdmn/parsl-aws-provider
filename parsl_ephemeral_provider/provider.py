...
class EphemeralProvider(ExecutionProvider, RepresentationMixin):
    ...
    compute_type : str, optional
        Type of compute resource when using serverless mode ('ec2', 'lambda', or
        'ecs'). Default is 'ec2'. ``'lambda'`` and ``'ecs'`` are rejected on the
        other two modes, which launch EC2 instances; leaving it at ``'ec2'`` on
        ``mode="serverless"`` warns, because ``ServerlessMode`` then falls back to
        its own ``'auto'`` selection rather than honouring the value.
    bastion_instance_type : str, optional
        Instance type for bastion host when using detached mode. Default is
        ``'t3.micro'``.  ``mode="detached"`` only.
    ...
    bastion_instance_profile_arn : str, optional
        Instance profile for the bastion to carry. Default is None, which creates
        one scoped to this workflow and deletes it with the bastion. A profile you
        supply is never deleted. Only used when ``bastion_host_type='direct'``;
        the CloudFormation path declares its own inside the stack.
        ``mode="detached"`` only.
    memory_size : int, optional
        Memory size in MB for Lambda functions. Default is 1024. The
        provider-facing name for ``ServerlessMode``'s ``lambda_memory``.
        ``mode="serverless"`` only.
    timeout : int, optional
        Timeout in seconds for Lambda functions. Default is 300; Lambda's own
        ceiling is 900. The provider-facing name for ``lambda_timeout``.
        ``mode="serverless"`` only.
    ...
    def __init__(
        ...
        memory_size: int = DEFAULT_LAMBDA_MEMORY,
        timeout: int = DEFAULT_LAMBDA_TIMEOUT,
        ...
        **kwargs: Any,
        ...
        # at their defaults.
        #
        # #155 extended these to the four options #136 left alone
        # (bastion_instance_type, memory_size, timeout, and compute_type). They
        # were mode-specific in exactly the same way but had been accepted since
        # before the guard existed, so covering them is a compatibility break --
        # deliberate, and recorded under Changed in the changelog. What it buys is
        # that there is no longer a second class of option that looks configured
        # and is not: `bastion_instance_type="m5.large"` on standard mode used to
        ...
        self._reject_wrong_mode_options(
            ...
            (
                ...
                # Lambda-only, and only meaningful on the serverless branch:
                # ServerlessMode maps them onto lambda_memory / lambda_timeout.
                ("memory_size", memory_size, DEFAULT_LAMBDA_MEMORY),
                ("timeout", timeout, DEFAULT_LAMBDA_TIMEOUT),
            ),
        )

        # compute_type does not fit _reject_wrong_mode_options, and the reason is
        # the whole difficulty #155 records. Its default is "ec2", which is
        # meaningful on standard and detached mode -- the only value that works
        # there -- and meaningless on serverless, where ServerlessMode ignores it
        # and falls back to its own "auto". So:
        #
        #   * a guard keyed on "value != default" would never fire for the case
        #     that actually confuses people, which is *leaving* it at "ec2" on
        #     serverless mode and expecting that to mean something; and
        #   * raising for that case is wrong, because it is the default -- a
        #     caller who never mentioned compute_type would start getting an
        #     error. Warn instead, naming what the mode will really do.
        #
        # The other direction is a plain error: "lambda"/"ecs" on a mode that
        # launches EC2 instances is a request the provider cannot honour at all.
        if self.mode_type == OperatingModeType.SERVERLESS:
            if self.compute_type == ComputeType.EC2:
                logger.warning(
                    "compute_type='ec2' has no meaning in serverless mode: it "
                    "leaves the mode on its own default of 'auto', which picks "
                    "Lambda for short single-task commands and ECS/Fargate "
                    "otherwise. Pass compute_type='lambda' or 'ecs' to choose "
                    "explicitly."
                )
        elif self.compute_type != ComputeType.EC2:
            raise ProviderConfigurationError(
                f"compute_type='{self.compute_type.value}' is supported only by "
                f"mode='serverless', not mode='{self.mode_type.value}'. "
                f"mode='{self.mode_type.value}' launches EC2 instances, so "
                "compute_type='ec2' is the only value it can honour."
            )
    ...
    def _initialize_operating_mode(self) -> OperatingMode:
        ...
        if self.mode_type == OperatingModeType.STANDARD:
            ...
            return DetachedMode(
                ...
                # None means "create one and delete it with the bastion" (#229),
                # so this is forwarded as-is too.
